...
Loco detects infinite loops (e.g. `(|a)*`) and [left recursion](http://en.wikipedia.org/wiki/Left_recursion) (e.g. `A -> Aa`) at grammar creation time.
...
### [json.php](https://github.com/ferno/loco/blob/master/json.php)

Parse [JSON](http://json.org/) expressions and returns PHP arrays.

### [regEx.php](https://github.com/ferno/loco/blob/master/regEx.php)

Parse simple regular expressions and return PHP objects representing them.

### [simpleComment.php](https://github.com/ferno/loco/blob/master/simpleComment.php)

Recognise simple valid HTML text using `<h5>`, `<p>`, `<em>` and `<strong>`, with balanced tags and escaped entities.



### [bnf.php](https://github.com/ferno/loco/blob/master/bnf.php)

Defines `$bnfGrammar`, which parses a grammar presented in [Backus-Naur Form](http://en.wikipedia.org/wiki/Backus%E2%80%93Naur_Form) and returns `Grammar` object capable of recognising that grammar.
...
### [wirth.php](https://github.com/ferno/loco/blob/master/wirth.php)

Defines `$wirthGrammar`, which parses a grammar presented in [Wirth syntax notation](http://en.wikipedia.org/wiki/Wirth_syntax_notation) and returns a `Grammar` object capable of recognising that grammar.
...
### [ebnf.php](https://github.com/ferno/loco/blob/master/ebnf.php)
...
`$ebnfGrammar` can't handle "specials" (strings contained between two question marks), since these have no clear definition. It also can't handle "exceptions" (when a `-` is used to discard certain possibilities), because these are not permissible in context-free grammars or possible with naive `MonoParser`s, and so would require special modification to Loco to handle.
...
### [locoNotation.php](https://github.com/ferno/loco/blob/master/locoNotation.php)
...
"Loco notation" (for lack of a better name) is an extension of Backus-Naur Form which gives access to all the `MonoParser`s that Loco makes available. The following parsers are already effectively available in most grammar notations:
...
Remember [simpleComment.php](https://github.com/ferno/loco/blob/master/simpleComment.php)? Here is that grammar in Loco notation.
...
Loco, together with the Loco grammar [simpleComment.php](https://github.com/ferno/loco/blob/master/simpleComment.php), fulfilled the first objective. These were kept in successful use for several years. Later, I developed [locoNotation.php](https://github.com/ferno/loco/blob/master/locoNotation.php) which things even simpler for me. However, there were some drawbacks:
